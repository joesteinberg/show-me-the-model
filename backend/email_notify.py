...
async def send_results_email(email: str, job_id: str, base_url: str):
    ...
    try:
        ...
        results_url = f"{base_url}/jobs/{job_id}"

        msg = MIMEMultipart("alternative")
        msg["Subject"] = "Your economic analysis is ready"
        msg["From"] = from_addr
        msg["To"] = email
...
<p style="color: #666; font-size: 0.9em;">
  ...
  Results are available for 24 hours.
        ...
        logger.info("Results email sent to %s for job %s", email, job_id)

    except Exception:
        logger.warning(
            "Failed to send email to %s for job %s", email, job_id, exc_info=True
        )
